	...
	*/	
	...
	function ambase_customizer_section( $wp_customize ) {
	    ...
	    $wp_customize->add_section(
	        'ambase_schema',
	        array(
	            'title'		  => __( 'Schema.org Images', 'ambase' ),
	            'description' => __( 'These images are used for schema.org blog markup. They are invisible on the page, but necessary for validation.', 'ambase' )
	        )
	    );

	    // Schema.org Images - Publisher Logo
		$wp_customize->add_setting(
		    'ambase_publisher_logo'
		);		
		$wp_customize->add_control(
		    new WP_Customize_Image_Control(
		        $wp_customize,
		        'ambase_publisher_logo',
		        array(
		            'label'	   => __( 'Publisher Logo', 'ambase' ),
		            'section'  => 'ambase_schema',
		            'settings' => 'ambase_publisher_logo',
		            'description' => __( 'This logo should be <strong>exactly 600px in width</strong> and <strong>no higher than 60px</strong>. See guidelines <a href="https://developers.google.com/search/docs/data-types/articles#amp-logo-guidelines"  target="_blank">here</a>.', 'ambase' )
		        )
		    )
		);

		// Schema.org Images - Publisher Logo Height
		$wp_customize->add_setting(
		    'ambase_publisher_logo_height',
		    array(
		        'sanitize_callback'	   => 'ambase_sanitize_number',
		        'sanitize_js_callback' => 'ambase_sanitize_number'
		    )
		...
		$wp_customize->add_control(
		    'ambase_publisher_logo_height',
		    array(
		        'label'	  => __( 'Logo Height', 'ambase' ),
		        'section' => 'ambase_schema',
		        'description' => __( 'Logo height in px - numeric value only.', 'ambase' ),
		        'type'	  => 'text'
		    )
		);

	    // Schema.org Images - Default Thumbnail
		$wp_customize->add_setting(
		    'ambase_default_thumbnail'
		);		
		$wp_customize->add_control(
		    new WP_Customize_Image_Control(
		        $wp_customize,
		        'ambase_default_thumbnail',
		        array(
		            'label'	   => __( 'Default Thumbnail', 'ambase' ),
		            'section'  => 'ambase_schema',
		            'settings' => 'ambase_default_thumbnail',
		            'description' => __( 'The default thumbnail for blog posts when there isn\'t one available.', 'ambase' )
		        )
		    )
		);

		// Schema.org Images - Default Thumbnail Width
		$wp_customize->add_setting(
		    'ambase_default_thumbnail_width',
		    array(
		        'sanitize_callback'	   => 'ambase_sanitize_number',
		        'sanitize_js_callback' => 'ambase_sanitize_number'
		    )
		...
		$wp_customize->add_control(
		    'ambase_default_thumbnail_width',
		    array(
		        'label'	  => __( 'Thumbnail Width', 'ambase' ),
		        'section' => 'ambase_schema',
		        'description' => __( 'Thumbnail width in px - numeric value only.', 'ambase' ),
		        'type'	  => 'text'
		    )
		);

		// Schema.org Images - Default Thumbnail Height
		$wp_customize->add_setting(
		    'ambase_default_thumbnail_height',
		    array(
		        'sanitize_callback'	   => 'ambase_sanitize_number',
		        'sanitize_js_callback' => 'ambase_sanitize_number'
		    )
		...
		$wp_customize->add_control(
		    'ambase_default_thumbnail_height',
		    array(
		        'label'	  => __( 'Thumbnail Height', 'ambase' ),
		        'section' => 'ambase_schema',
		        'description' => __( 'Thumbnail height in px - numeric value only.', 'ambase' ),
		        'type'	  => 'text'
		    )
		);
